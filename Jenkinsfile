...
pipeline {
  ...
  stages {
    stage('Checkout') {
      steps {
        checkout scm
        echo "Starting Performance Testing Pipeline for Master"
      }
    }
      ...
      steps {
        sh """
          ...
          timeout 300 docker exec ${JMETER_CONTAINER_NAME} jmeter -n \
            -t /work/jmeter/trabajo-Integrador.jmx \
...
<body>
    <div class="header">
        ...
        <p><strong>Build:</strong> #${BUILD_NUMBER} | <strong>Branch:</strong> Master | <strong>Date:</strong> \$(date)</p>
      ...
      steps {
        script {
          if (fileExists("${OUT_DIR}/results.jtl")) {
            ...
            def roundedSuccessRate = String.format("%.1f", successRate)
            currentBuild.description = "Success: ${roundedSuccessRate}% | Avg: ${avgResponse}ms | Max: ${maxResponse}ms | Requests: ${results}"
